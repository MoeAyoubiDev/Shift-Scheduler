-- =====================================================
-- SHIFT SCHEDULER SYSTEM - COMPLETE DATABASE SCHEMA
-- =====================================================
-- Production-ready MySQL schema with stored procedures
-- All business logic implemented in stored procedures
-- =====================================================
...
CREATE TABLE weeks (
    ...
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY unique_week_start (week_start_date)
);
...
CREATE TABLE schedules (
    ...
    UNIQUE KEY unique_schedule_week_section (week_id, section_id)
);
...
CREATE TABLE schedule_assignments (
    ...
    employee_id INT NOT NULL,
    assignment_source VARCHAR(20) NOT NULL DEFAULT 'MATCHED_REQUEST' CHECK (assignment_source IN ('MATCHED_REQUEST','AUTO_ASSIGNED','MANUALLY_ADJUSTED')),
    is_senior TINYINT(1) DEFAULT 0,
...
CREATE TABLE notifications (
    id INT AUTO_INCREMENT PRIMARY KEY,
    user_id INT NOT NULL,
    type VARCHAR(20) NOT NULL CHECK (type IN ('SHIFT_REMINDER','SCHEDULE_PUBLISHED','REQUEST_STATUS')),
...
CREATE PROCEDURE sp_verify_login(IN p_username VARCHAR(100))
BEGIN
    ...
    WHERE u.username = p_username AND u.is_active = 1
    ORDER BY ur.id;
END$$
...
BEGIN
    SELECT id AS shift_type_id, code, name AS shift_type_name, start_time, end_time
    FROM shift_types
    ORDER BY id;
END$$
...
-- Shift Requests (Sunday blocked, Senior cannot submit)
...
BEGIN
    ...
    -- Check if Sunday (0 = Sunday, 1 = Monday, etc.)
    ...
    -- Check if employee is Senior (Seniors cannot submit requests)
...
BEGIN
    ...
    ORDER BY sr.shift_date, sr.shift_type_id;
END$$
...
BEGIN
    ...
    ON DUPLICATE KEY UPDATE 
        generated_at = NOW(), 
        generated_by_admin_id = p_generated_by_employee_id,
        status = 'DRAFT';
    ...
    -- Delete existing assignments and shifts
    ...
    -- Create schedule shifts from requirements
    ...
    -- Auto-assign remaining slots based on seniority
...
BEGIN
    ...
    -- Get current schedule_shift_id
    SELECT schedule_shift_id INTO v_schedule_shift_id
    FROM schedule_assignments
    WHERE id = p_assignment_id;

    -- Find or create new schedule_shift with the new definition
...
-- Break Management (30-minute breaks only)
...
BEGIN
    ...
    -- Check if break already exists for today
...
BEGIN
    SELECT eb.id AS break_id,
           ...
           eb.is_active,
           CASE
               WHEN eb.break_end IS NULL AND eb.break_start IS NOT NULL THEN 
                   GREATEST(TIMESTAMPDIFF(MINUTE, eb.break_start, NOW()) - 30, 0)
               WHEN eb.break_end IS NOT NULL THEN 
                   GREATEST(TIMESTAMPDIFF(MINUTE, eb.break_start, eb.break_end) - 30, 0)
               ELSE 0
           END AS delay_minutes,
           CASE
               WHEN eb.is_active = 1 THEN 'On Break'
               WHEN eb.break_start IS NULL THEN 'Not Started'
...
BEGIN
    SELECT e.id AS employee_id,
           ...
           COALESCE(SUM(GREATEST(
               TIMESTAMPDIFF(MINUTE, eb.break_start, eb.break_end) - 30, 0
...
BEGIN
    SELECT 'Total Employees' AS metric_label,
           COUNT(DISTINCT e.id) AS metric_value,
    ...
    SELECT 'Pending Requests',
           COUNT(*) AS metric_value,
           'Requests pending approval' AS description
    FROM shift_requests sr
    INNER JOIN employees e ON e.id = sr.employee_id
    INNER JOIN user_roles ur ON ur.id = e.user_role_id
    WHERE sr.week_id = p_week_id AND ur.section_id = p_section_id AND sr.status = 'PENDING'
    UNION ALL
    SELECT 'Total Breaks Today',
           COUNT(*) AS metric_value,
           'Breaks logged today' AS description
    FROM employee_breaks eb
    INNER JOIN employees e ON e.id = eb.employee_id
    INNER JOIN user_roles ur ON ur.id = e.user_role_id
...
-- Sample Director user (password: director123)
INSERT INTO users (username, password_hash, email) VALUES
('director', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'director@company.com');

SET @director_user_id = LAST_INSERT_ID();
SET @director_role_id = (SELECT id FROM roles WHERE role_name = 'Director');
SET @app_section_id = (SELECT id FROM sections WHERE section_name = 'App After-Sales');
SET @agent_section_id = (SELECT id FROM sections WHERE section_name = 'Agent After-Sales');

-- Director has access to both sections
INSERT INTO user_roles (user_id, role_id, section_id) VALUES
(@director_user_id, @director_role_id, @app_section_id),
(@director_user_id, @director_role_id, @agent_section_id);

-- Sample Team Leader (password: teamleader123)
INSERT INTO users (username, password_hash, email) VALUES
('teamleader', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'teamleader@company.com');

SET @tl_user_id = LAST_INSERT_ID();
SET @tl_role_id = (SELECT id FROM roles WHERE role_name = 'Team Leader');

INSERT INTO user_roles (user_id, role_id, section_id) VALUES
(@tl_user_id, @tl_role_id, @app_section_id);

SET @tl_user_role_id = LAST_INSERT_ID();

INSERT INTO employees (user_role_id, employee_code, full_name, email, is_senior, seniority_level) VALUES
(@tl_user_role_id, 'TL001', 'Team Leader One', 'teamleader@company.com', 0, 10);

-- Sample Employee (password: employee123)
INSERT INTO users (username, password_hash, email) VALUES
('employee', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'employee@company.com');

SET @emp_user_id = LAST_INSERT_ID();
SET @emp_role_id = (SELECT id FROM roles WHERE role_name = 'Employee');

INSERT INTO user_roles (user_id, role_id, section_id) VALUES
(@emp_user_id, @emp_role_id, @app_section_id);

SET @emp_user_role_id = LAST_INSERT_ID();

INSERT INTO employees (user_role_id, employee_code, full_name, email, is_senior, seniority_level) VALUES
(@emp_user_role_id, 'EMP001', 'Employee One', 'employee@company.com', 0, 5);

-- Note: All passwords above are hashed versions of "password" for testing
-- In production, use proper password hashing with password_hash() function
